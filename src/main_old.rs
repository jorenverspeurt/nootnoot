...
impl HostStats {
    fn new(address: String, log_buffer_size: usize, rolling_window_size: usize) -> Self {
        HostStats {
            address,
            reachable: 0,
            unreachable: 0,
            total_latency: 0,
            count: 0,
            rolling_latency: Vec::with_capacity(rolling_window_size),
            rolling_window_size,
    ...
    fn update(
        ...
        match result {
            Some(latency) => {
                ...
                if self.rolling_latency.len() == self.rolling_window_size {
                    self.rolling_latency.remove(0);
                }
                self.rolling_latency.push(latency_ms);
    ...
    fn rolling_average_latency(&self) -> Option<u64> {
        if !self.rolling_latency.is_empty() {
            Some(self.rolling_latency.iter().sum::<u64>() / self.rolling_latency.len() as u64)
        } else {
            None
        }
    }

    fn rolling_standard_deviation(&self) -> Option<f64> {
        // TODO: Set a more informed minimum sample number here
        if self.rolling_latency.len() > 10 {
            let mean = self.rolling_average_latency().unwrap_or(0) as f64;
            let variance: f64 = self
                .rolling_latency
                .iter()
                .map(|&lat| (lat as f64 - mean).powi(2))
                .sum::<f64>()
                / self.rolling_latency.len() as f64;
            Some(variance.sqrt())
        } else {
            None
        }
    ...
    fn check_anomalies(&mut self, latency: u64, stddev_threshold: f64) -> bool {
        if let Some(mean) = self.rolling_average_latency() {
            if let Some(stddev) = self.rolling_standard_deviation() {
                return (latency as f64) > mean as f64 + stddev_threshold * stddev;
            }
        }
        false
    }
...
impl Serialize for HostStats {
        ...
        S: Serializer,
    {
        ...
        state.serialize_field("rolling_latency", &self.rolling_latency)?;
        state.serialize_field("rolling_window_size", &self.rolling_window_size)?;
...
async fn run_service(
    ...
    for host in config.hosts {
        ...
        let rolling_window_size: usize = 1000;
        ...
        handles.push(tokio::spawn(async move {
            ...
            while !stop_flag.load(Ordering::Relaxed) {
                ...
                let host_stats = stats_guard.entry(host.address.clone()).or_insert_with(|| HostStats::new(host.address.clone(), log_buffer_size, rolling_window_size));
                ...
                if verbose {
                    ...
                    let anomalous = host_stats.check_anomalies(result.map(|r| r.as_millis() as u64).unwrap_or(0), config.stddev_threshold.unwrap_or(2.0));
                    if anomalous {
                        let log_entry = format!(
                            "[{}] {} latency {}ms > rolling mean {}ms + 2 * stddev {}ms",
                            time_now,
                            host.address,
                            result.map(|r| r.as_millis()).unwrap_or(0),
                            host_stats.rolling_average_latency().map_or("N/A".to_string(), |mean| mean.to_string()),
                            host_stats.rolling_standard_deviation().map_or("N/A".to_string(), |stddev| format!("{:.2}", stddev))
                        );
                        write_log(&log_path, &log_entry, stdout);
                    }
                }
    ...
    tokio::spawn(async move {
        warp::serve(warp::path("dashboard").and_then(move || {
            ...
            async move {
                ...
                let response: Vec<_> = stats_guard
                    .iter()
                    .map(|(host, stats)| {
                        ...
                        format!(
                            ...
                            stats
                                .average_latency()
                                .map_or("N/A".to_string(), |lat| lat.to_string()),
                            stats.reachable_percentage(),
                            unavail_log
                        )
                    })
                    .collect();
